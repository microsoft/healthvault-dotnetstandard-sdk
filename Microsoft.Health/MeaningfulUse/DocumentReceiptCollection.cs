...
{
    /// <summary>
    /// A collection of <see cref="DocumentReceipt" />(s) which describe document receipt information
    /// for Meaningful Use Timely Access Report from HealthVault. 
    /// </summary>
    internal class DocumentReceiptCollection : ReportCollection<DocumentReceipt>
    {
        private HealthServiceConnection _connection;
        private DateRange _reportingPeriodFilter;

        /// <summary>
        /// Constructs an instance of DocumentReceiptCollection with specified values.
        /// </summary>
        /// 
        ...
        /// </param>
        /// 
        internal DocumentReceiptCollection(
        ...
        private void ParseResponse(XPathNavigator responseInfoNavigator)
        {
            ...
            if (sourceNavs != null)
            {
                foreach (XPathNavigator sourceNav in sourceNavs)
                {
                    ...
                    XPathNodeIterator activityNavs = sourceNav.Select("document-receipts/document-receipt");

                    if (sourceNavs != null)
                    {
                        foreach (XPathNavigator activityNav in activityNavs)
                        {
                            DocumentReceipt item = new DocumentReceipt(source);
        ...
        private HealthServiceRequest CreateRequest()
        {
            ...
            {
                writer.WriteStartElement("document-filter");
